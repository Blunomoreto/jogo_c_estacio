...
void desenhar_hud(Jogo *jogo)
{
    ...
    {
        char ammo[32];
        snprintf(ammo, sizeof(ammo), "Missiles: %d/%d", jogo->jogador.municao_guiada, jogo->jogador.municao_guiada_max);
    ...
    {
        char msg[128];
        snprintf(msg, sizeof(msg), "Upgrade: %s", jogo->ultima_melhoria);
...
void desenhar_vida_boss(Jogo *jogo)
{
    ...
    for (i = 0; i < MAXIMO_INIMIGOS; ++i)
    {
        ...
        if (e->ativo && e->eh_chefao)
        {
            float pct = e->vida / e->vida_maxima;
            if (pct < 0.0f)
                pct = 0.0f;
            renderizar_retangulo(jogo->largura * 0.5f - 250.0f, 20.0f, 500.0f, 16.0f, (Cor){0.15f, 0.12f, 0.2f, 0.92f});
            renderizar_retangulo(jogo->largura * 0.5f - 250.0f, 20.0f, 500.0f * pct, 16.0f, (Cor){0.95f, 0.2f, 0.85f, 0.95f});
...
void desenhar_menu(Jogo *jogo)
{
    ...
    float mx = (float)jogo->entrada.mouse_x;
    float my = (float)jogo->entrada.mouse_y;
    float pulse = 0.5f + 0.5f * sinf(jogo->tempo_decorrido * 6.0f);
...

    {
        ...
        for (idx = 0; idx < 3; ++idx)
        {
            hoverAnim[idx] += (targets[idx] - hoverAnim[idx]) * (8.0f * jogo->tempo_delta);
    ...
    if (hoverAnim[0] > 0.01f)
        renderizar_retangulo(bx - shadowPad - 4.0f * uiScale, by - 9.0f * uiScale,
                             ...
                             (Cor){0.45f, 0.75f, 1.0f, (0.10f + 0.18f * pulse) * hoverAnim[0]});
    renderizar_retangulo(bx - shadowPad, by - 5.0f * uiScale, btnW + shadowPad * 2.0f, startH + 10.0f * uiScale,
                         (Cor){0.08f + 0.12f * hoverAnim[0], 0.25f + 0.20f * hoverAnim[0], 0.55f + 0.35f * hoverAnim[0], 0.50f + 0.05f * hoverAnim[0]});
    renderizar_retangulo(bx, by, btnW, startH,
                         (Cor){0.15f + 0.07f * hoverAnim[0], 0.45f + 0.13f * hoverAnim[0], 0.85f + 0.15f * hoverAnim[0], 0.95f + 0.03f * hoverAnim[0]});
    ...
    if (hoverAnim[1] > 0.01f)
        renderizar_retangulo(obx - shadowPad - 3.0f * uiScale, oby - 8.0f * uiScale,
                             ...
                             (Cor){0.78f, 0.52f, 1.0f, (0.08f + 0.16f * pulse) * hoverAnim[1]});
    renderizar_retangulo(obx - shadowPad, oby - 5.0f * uiScale, btnW + shadowPad * 2.0f, subH + 10.0f * uiScale,
                         (Cor){0.26f + 0.15f * hoverAnim[1], 0.12f + 0.07f * hoverAnim[1], 0.36f + 0.24f * hoverAnim[1], 0.50f + 0.05f * hoverAnim[1]});
    renderizar_retangulo(obx, oby, btnW, subH,
                         (Cor){0.36f + 0.12f * hoverAnim[1], 0.22f + 0.10f * hoverAnim[1], 0.62f + 0.20f * hoverAnim[1], 0.90f + 0.05f * hoverAnim[1]});
    ...
    if (hoverAnim[2] > 0.01f)
        renderizar_retangulo(lbx - shadowPad - 3.0f * uiScale, lby - 8.0f * uiScale,
                             btnW + (shadowPad + 3.0f * uiScale) * 2.0f, subH + 16.0f * uiScale,
                             (Cor){0.45f, 0.95f, 0.90f, (0.08f + 0.16f * pulse) * hoverAnim[2]});
    renderizar_retangulo(lbx - shadowPad, lby - 5.0f * uiScale, btnW + shadowPad * 2.0f, subH + 10.0f * uiScale,
                         (Cor){0.08f + 0.08f * hoverAnim[2], 0.22f + 0.18f * hoverAnim[2], 0.30f + 0.20f * hoverAnim[2], 0.50f + 0.05f * hoverAnim[2]});
    renderizar_retangulo(lbx, lby, btnW, subH,
                         (Cor){0.16f + 0.10f * hoverAnim[2], 0.36f + 0.16f * hoverAnim[2], 0.50f + 0.18f * hoverAnim[2], 0.90f + 0.05f * hoverAnim[2]});
    ...
    renderizar_retangulo(jogo->largura * 0.5f - 400.0f, jogo->altura * 0.90f, 800.0f, compactUI ? 40.0f : 60.0f, (Cor){0.0f, 0.0f, 0.0f, 0.25f});
    ...
    {
        char stats[128];
        snprintf(stats, sizeof(stats), "High Score: %d", jogo->recorde_pontuacao_maxima);
        desenhar_texto(50.0f, jogo->altura * 0.82f, stats, GLUT_BITMAP_HELVETICA_18, 1.0f, 0.9f, 0.4f);
        snprintf(stats, sizeof(stats), "Max Wave: %d", jogo->recorde_onda_maxima);
    ...
    renderizar_retangulo(jogo->largura - 350.0f, jogo->altura * 0.78f, 320.0f, 90.0f, (Cor){0.0f, 0.0f, 0.0f, 0.4f});
    {
        ...
        snprintf(cfg, sizeof(cfg), "Audio: %s", jogo->audio_habilitado ? "ON" : "OFF");
        desenhar_texto(jogo->largura - 330.0f, jogo->altura * 0.86f, cfg, GLUT_BITMAP_HELVETICA_18, 0.3f, jogo->audio_habilitado ? 0.9f : 0.5f, 0.8f);
    }
    ...
    {
        char row[128];
        snprintf(row, sizeof(row), "%d. %-18s  %7d  W%d", s + 1, jogo->melhores_pontuacoes[s].nome, jogo->melhores_pontuacoes[s].pontuacao, jogo->melhores_pontuacoes[s].onda);
...
void desenhar_placar(Jogo *jogo)
{
    ...
    int startIndex = jogo->pagina_pontuacao * jogo->itens_por_pagina_pontuacao;
    int endIndex = startIndex + jogo->itens_por_pagina_pontuacao;
    char pageText[64];
...

    {
        ...
        for (hi = 0; hi < 4; ++hi)
        {
            scoreHover[hi] += (targets[hi] - scoreHover[hi]) * (8.0f * jogo->tempo_delta);
    ...
    if (pageCount <= 0)
        pageCount = 1;
    if (endIndex > jogo->numero_todas_pontuacoes)
        endIndex = jogo->numero_todas_pontuacoes;
    ...
    else
    {
        ...
        for (i = startIndex; i < endIndex; ++i)
        {
            ...
            snprintf(row, sizeof(row), "%02d  %-22s %8d   %4d", i + 1, jogo->todas_pontuacoes[i].nome, jogo->todas_pontuacoes[i].pontuacao, jogo->todas_pontuacoes[i].onda);
            desenhar_texto(jogo->largura * 0.5f - 270.0f, (float)rowY, row, GLUT_BITMAP_HELVETICA_18, 0.85f, 0.93f, 1.0f);
        }
    }

    snprintf(pageText, sizeof(pageText), "Page %d/%d", jogo->pagina_pontuacao + 1, pageCount);
    ...
    if (scoreHover[0] > 0.01f)
        renderizar_retangulo(centerX - 258.0f, jogo->altura * 0.20f - 3.0f, 166.0f, 50.0f, (Cor){0.48f, 0.68f, 1.0f, (0.10f + 0.12f * pulse) * scoreHover[0]});
    renderizar_retangulo(jogo->largura * 0.5f - 255.0f, jogo->altura * 0.20f, 160.0f, 44.0f,
                         (Cor){0.20f + 0.08f * scoreHover[0], 0.30f + 0.10f * scoreHover[0], 0.52f + 0.16f * scoreHover[0], 0.90f + 0.05f * scoreHover[0]});
    ...
    if (scoreHover[1] > 0.01f)
        renderizar_retangulo(centerX + 92.0f, jogo->altura * 0.20f - 3.0f, 166.0f, 50.0f, (Cor){0.48f, 0.68f, 1.0f, (0.10f + 0.12f * pulse) * scoreHover[1]});
    renderizar_retangulo(jogo->largura * 0.5f + 95.0f, jogo->altura * 0.20f, 160.0f, 44.0f,
                         (Cor){0.20f + 0.08f * scoreHover[1], 0.30f + 0.10f * scoreHover[1], 0.52f + 0.16f * scoreHover[1], 0.90f + 0.05f * scoreHover[1]});
    ...
    if (scoreHover[2] > 0.01f)
        renderizar_retangulo(centerX - 113.0f, jogo->altura * 0.20f - 3.0f, 226.0f, 50.0f, (Cor){1.0f, 0.52f, 0.54f, (0.10f + 0.12f * pulse) * scoreHover[2]});
    renderizar_retangulo(jogo->largura * 0.5f - 110.0f, jogo->altura * 0.20f, 220.0f, 44.0f,
                         (Cor){0.45f + 0.10f * scoreHover[2], 0.20f + 0.06f * scoreHover[2], 0.22f + 0.08f * scoreHover[2], 0.90f + 0.05f * scoreHover[2]});
    ...
    if (scoreHover[3] > 0.01f)
        renderizar_retangulo(centerX - 113.0f, jogo->altura * 0.11f - 3.0f, 226.0f, 50.0f, (Cor){0.52f, 0.78f, 1.0f, (0.10f + 0.12f * pulse) * scoreHover[3]});
    renderizar_retangulo(jogo->largura * 0.5f - 110.0f, jogo->altura * 0.11f, 220.0f, 44.0f,
                         (Cor){0.10f + 0.08f * scoreHover[3], 0.35f + 0.12f * scoreHover[3], 0.70f + 0.14f * scoreHover[3], 0.90f + 0.05f * scoreHover[3]});
...
void desenhar_opcoes(Jogo *jogo)
{
...

    {
        ...
        for (oi = 0; oi < 3; ++oi)
        {
            optHover[oi] += (targets[oi] - optHover[oi]) * (8.0f * jogo->tempo_delta);
            if (optHover[oi] < 0.0f)
                optHover[oi] = 0.0f;
            if (optHover[oi] > 1.0f)
                optHover[oi] = 1.0f;
    ...
    if (optHover[0] > 0.01f)
        renderizar_retangulo(centerX - 183.0f, jogo->altura * 0.56f - 3.0f, 366.0f, 62.0f, (Cor){0.52f, 0.80f, 1.0f, (0.10f + 0.12f * pulse) * optHover[0]});
    renderizar_retangulo(jogo->largura * 0.5f - 180.0f, jogo->altura * 0.56f, 360.0f, 56.0f,
                         (Cor){0.20f + 0.08f * optHover[0], 0.25f + 0.10f * optHover[0], 0.50f + 0.16f * optHover[0], 0.90f + 0.05f * optHover[0]});
    snprintf(linha, sizeof(linha), "Audio: %s  (click or A)", jogo->audio_habilitado ? "ON" : "OFF");
    desenhar_texto(jogo->largura * 0.5f - 150.0f, jogo->altura * 0.595f, linha, GLUT_BITMAP_HELVETICA_18, 1.0f, 1.0f, 1.0f);

    if (optHover[1] > 0.01f)
        renderizar_retangulo(centerX - 183.0f, jogo->altura * 0.46f - 3.0f, 366.0f, 62.0f, (Cor){0.84f, 0.66f, 1.0f, (0.10f + 0.12f * pulse) * optHover[1]});
    renderizar_retangulo(jogo->largura * 0.5f - 180.0f, jogo->altura * 0.46f, 360.0f, 56.0f,
                         (Cor){0.28f + 0.10f * optHover[1], 0.22f + 0.08f * optHover[1], 0.44f + 0.15f * optHover[1], 0.90f + 0.05f * optHover[1]});
    ...
    if (optHover[2] > 0.01f)
        renderizar_retangulo(centerX - 113.0f, jogo->altura * 0.31f - 3.0f, 226.0f, 56.0f, (Cor){0.52f, 0.80f, 1.0f, (0.10f + 0.12f * pulse) * optHover[2]});
    renderizar_retangulo(jogo->largura * 0.5f - 110.0f, jogo->altura * 0.31f, 220.0f, 50.0f,
                         (Cor){0.10f + 0.08f * optHover[2], 0.35f + 0.12f * optHover[2], 0.70f + 0.14f * optHover[2], 0.90f + 0.05f * optHover[2]});
...
void desenhar_pausa(Jogo *jogo)
{
    ...
    renderizar_retangulo(centerX - 240.0f, jogo->altura * 0.42f, 480.0f, compactUI ? 110.0f : 140.0f, (Cor){0.06f, 0.10f, 0.18f, 0.88f});
...
void desenhar_melhorias_tela(Jogo *jogo)
{
    ...
    Vetor2D mouse = matematica_vetor2d((float)jogo->entrada.mouse_x, (float)jogo->entrada.mouse_y);
    float centerX = jogo->largura * 0.5f;
    float uiScale = (float)jogo->largura / 1280.0f;
    float hScale = (float)jogo->altura / 720.0f;
    ...
    renderizar_retangulo(centerX - totalW * 0.5f - 40.0f * uiScale, cardY - 70.0f * uiScale,
                         totalW + 80.0f * uiScale, cardH + 190.0f * uiScale, (Cor){0.07f, 0.10f, 0.18f, 0.86f});
    ...
    for (i = 0; i < MAXIMO_OPCOES_UPGRADE; ++i)
    {
        ...
        if (hovered)
            jogo->melhoria_selecionada = i;
        upgradeHoverAnim[i] += ((hovered ? 1.0f : 0.0f) - upgradeHoverAnim[i]) * (8.0f * jogo->tempo_delta);
        ...
        if (upgradeHoverAnim[i] > 0.01f)
            renderizar_retangulo(bx - 4.0f * uiScale, by - 4.0f * uiScale, cardW + 8.0f * uiScale, cardH + 8.0f * uiScale,
                                 (Cor){0.65f, 0.88f, 1.0f, (0.10f + 0.14f * pulse) * upgradeHoverAnim[i]});
        renderizar_retangulo(bx, by, cardW, cardH,
                             (Cor){0.14f + 0.06f * upgradeHoverAnim[i], 0.20f + i * 0.07f + 0.06f * upgradeHoverAnim[i], 0.45f + 0.10f * upgradeHoverAnim[i], 0.92f + 0.04f * upgradeHoverAnim[i]});
...

    {
        ...
        rerollHoverAnim += ((rerollHovered ? 1.0f : 0.0f) - rerollHoverAnim) * (8.0f * jogo->tempo_delta);
        ...
        if (rerollHoverAnim > 0.01f)
            renderizar_retangulo(rerollX - 3.0f * uiScale, rerollY - 3.0f * uiScale, rerollW + 6.0f * uiScale, rerollH + 6.0f * uiScale,
                                 (Cor){0.75f, 0.64f, 1.0f, (0.08f + 0.12f * pulse) * rerollHoverAnim});
        renderizar_retangulo(rerollX, rerollY, rerollW, rerollH,
                             (Cor){0.22f + 0.10f * rerollHoverAnim, 0.18f + 0.08f * rerollHoverAnim, 0.35f + 0.12f * rerollHoverAnim, 0.92f + 0.04f * rerollHoverAnim});
    ...
    {
        renderizar_retangulo(centerX - 240.0f * uiScale, rerollY + rerollH + 12.0f * uiScale, 480.0f * uiScale, 40.0f * uiScale, (Cor){0.08f, 0.12f, 0.24f, 0.85f});
        desenhar_texto(centerX - 220.0f * uiScale, rerollY + rerollH + 36.0f * uiScale, jogo->melhorias[jogo->melhoria_selecionada].descricao, GLUT_BITMAP_HELVETICA_18, 0.95f, 0.98f, 1.0f);
    }
}
...
void desenhar_fim(Jogo *jogo)
{
    ...
    float my = (float)jogo->entrada.mouse_y;
    float pulse = 0.5f + 0.5f * sinf(jogo->tempo_decorrido * 6.0f);
    int hoverMenu = (jogo->nome_salvo && mx >= centerX - 110.0f && mx <= centerX + 110.0f && my >= jogo->altura * 0.45f && my <= jogo->altura * 0.45f + 50.0f);
    char linha[160];

    endMenuHover += ((hoverMenu ? 1.0f : 0.0f) - endMenuHover) * (8.0f * jogo->tempo_delta);
    ...
    renderizar_retangulo(0.0f, 0.0f, (float)jogo->largura, (float)jogo->altura, (Cor){0.0f, 0.0f, 0.0f, 0.62f});
    renderizar_retangulo(centerX - 280.0f, jogo->altura * 0.38f, 560.0f, 260.0f, (Cor){0.08f, 0.10f, 0.18f, 0.86f});
    ...
    if (!jogo->nome_salvo)
    {
        ...
        renderizar_retangulo(centerX - 170.0f, jogo->altura * 0.61f, 340.0f, 40.0f, (Cor){0.12f, 0.12f, 0.2f, 0.9f});
        desenhar_texto(centerX - 160.0f, jogo->altura * 0.635f, jogo->nome_jogador, GLUT_BITMAP_HELVETICA_18, 1.0f, 1.0f, 0.8f);
    }
    else
    {
        ...
        if (endMenuHover > 0.01f)
            renderizar_retangulo(centerX - 113.0f, jogo->altura * 0.45f - 3.0f, 226.0f, 56.0f,
                                 (Cor){0.52f, 0.80f, 1.0f, (0.10f + 0.12f * pulse) * endMenuHover});
        renderizar_retangulo(centerX - 110.0f, jogo->altura * 0.45f, 220.0f, 50.0f,
                             (Cor){0.10f + 0.10f * endMenuHover, 0.35f + 0.12f * endMenuHover, 0.70f + 0.14f * endMenuHover, 0.90f + 0.05f * endMenuHover});
