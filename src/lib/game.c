...
static void spawn_wave(Game *g)
{
    ...
    for (i = 0; i < amount; ++i)
    {
        ...
        e->angularSpeed = (e->isBoss ? randf(0.45f, 0.85f) : randf(0.75f, 1.6f)) * ((i % 2 == 0) ? 1.0f : -1.0f) * (1.0f + g->wave * 0.08f);
...
static void update_playing(Game *g, float dt)
{
    ...
    for (i = 0; i < MAX_PROJECTILES; ++i)
    {
        ...
        for (oi = 0; oi < MAX_OBSTACLES && p->active; ++oi)
        {
            ...
            if (!o->active)
                continue;
            if (circle_vs_aabb(p->pos, p->radius, vec2(o->x, o->y), vec2(o->x + o->w, o->y + o->h)))
                p->active = 0;
        }
    }
    ...
    for (i = 0; i < MAX_ENEMIES; ++i)
    {
        ...
        if (e->shootCooldown <= 0.0f)
        {
            ...
            else if (e->type == ENEMY_PENTAGON)
            {
                ...
                spawn_projectile(g, ep, dirToPlayer, 0, 300.0f + g->wave * 15.0f, 30.0f + g->wave * 5.0f, 6.0f, 6.0f, GUIDANCE_APNG, -1, apnOverload);
